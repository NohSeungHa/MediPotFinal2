...
public interface HelpZoneService {
	...
	int selectTitleSearchCount(String searchContent);	// 검색한 제목 갯수
	int selectContentSearchCount(String searchContent);	// 검색한 내용 갯수
	
	List<HelpZone> selectHelpZoneList(int cPage, int numPerPage);	//헬프존 리스트 출력하기
	List<HelpZone> selectHelpZoneTitleList(int cPage, int numPerPage, String searchContent);	// 제목으로 검색
	List<HelpZone> selectHelpZoneContentList(int cPage, int numPerPage, String searchContent);	// 내용으로 검색
